...
export class FormlyValidation {
  ...
  private minlengthValidationMessage(_err:any, field: FormlyFieldConfig, translate: TranslateService) {
    return translate.stream('validations.minLength', { number: field.templateOptions.minLength });
  }

  private maxlengthValidationMessage(_err:any, field: FormlyFieldConfig, translate: TranslateService) {
    return translate.stream('validations.maxLength', { number: field.templateOptions.maxLength });
  }

  private minValidationMessage(_err:any, field: FormlyFieldConfig, translate: TranslateService) {
    return translate.stream('validations.min', { number: field.templateOptions.min });
  }

  private maxValidationMessage(_err:any, field: FormlyFieldConfig, translate: TranslateService) {
